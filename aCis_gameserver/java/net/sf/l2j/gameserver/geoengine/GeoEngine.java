...
public class GeoEngine
{
	...
	public final Location getValidLocation(WorldObject follower, int tx, int ty, int tz)
	{
		return getValidLocation(follower.getX(), follower.getY(), follower.getZ(), tx, ty, tz);
	}
	...
	public final Location getValidLocation(Location originLoc, Location targetLoc)
	{
		return getValidLocation(originLoc.getX(), originLoc.getY(), originLoc.getZ(), targetLoc.getX(), targetLoc.getY(), targetLoc.getZ());
	}
	 ...
	 */
	public final Location getValidLocation(int ox, int oy, int oz, int tx, int ty, int tz)
	{
		// get origin and check existing geo coordinates
		final int gox = getGeoX(ox);
		final int goy = getGeoY(oy);
	...
	protected final GeoLocation checkMove(int gox, int goy, int goz, int gtx, int gty, int gtz)
	{
		...
		do
		{
			...
			final int maxdz = 50;
			final int nz = getHeightNearest(nx, ny, gpz);
			if (Math.abs(nz - gpz) > maxdz)
				return new GeoLocation(gpx, gpy, gpz);
	 ...
	 */
	public LinkedList<Location> findPath(int ox, int oy, int oz, int tx, int ty, int tz, boolean playable)
	{
