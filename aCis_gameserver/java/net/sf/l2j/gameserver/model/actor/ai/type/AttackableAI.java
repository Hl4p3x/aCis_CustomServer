...
public class AttackableAI extends CreatureAI implements Runnable
{
	...
	private void thinkCast()
	{
		...
		if (_actor.getMove().maybeMoveToPawn(getTarget(), _skill.getCastRange()))
			return;
	...
	protected void thinkActive()
	{
		...
		else
		{
			...
			if (npc.getSpawn() != null && !npc.isNoRndWalk() && Rnd.get(Config.RANDOM_WALK_RATE) == 0)
			{
				final Location loc = npc.getMove().getDestination();
				loc.setUsingRandomOffset(npc.getSpawn().getLoc(), Config.MAX_DRIFT_RANGE);
				
				npc.getMove().moveToLocation(loc);
			}
		}
	}
	...
	protected void thinkAttack()
	{
			...
			if (!defaultList.isEmpty())
			{
				...
				if (master != null && !master.isDead() && (master.getCurrentHp() / master.getMaxHp() < 0.75))
				{
					for (L2Skill sk : defaultList)
					{
						...
						{
							npc.getMove().moveToPawn(master, overallRange);
							return;
						}
		...
		if (Rnd.get(100) <= 3)
		{
			for (Attackable nearby : npc.getKnownTypeInRadius(Attackable.class, actorCollision))
			{
				...
				if (!npc.isInsideRadius(newX, newY, actorCollision, false))
					npc.getMove().moveToLocation(newX, newY, npc.getZ() + 30);
		...
		{
			final Location loc = npc.getMove().getDestination();
			loc.setFleeing(npc, attackTarget, Config.MAX_DRIFT_RANGE);
		...
		if (!npc.getMove().maybeMoveToPawn(attackTarget, npc.getPhysicalAttackRange()))
			npc.doAttack(attackTarget);
	}
	...
	protected void onEvtThink()
	{
		...
		if (_thinking || _actor.isAllSkillsDisabled() || _actor.cantAttack())
			return;
	...
	{
		if (Rnd.get(Config.RANDOM_WALK_RATE) != 0)
			return false;
