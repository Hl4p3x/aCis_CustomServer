...
public class PetStat extends SummonStat
{
	...
	{
		return (int) calcStat(Stats.MAGIC_ATTACK, getActiveChar().getPetData().getMAtk(), target, skill);
	}
	...
	{
		final PetDataEntry pde = getActiveChar().getTemplate().getPetDataEntry(level);
		if (pde == null)
			return 0;
	...
	{
		final PetDataEntry pde = getActiveChar().getTemplate().getPetDataEntry(getLevel());
		if (pde == null)
			return 0;
	...
	{
		final PetDataEntry pde = getActiveChar().getTemplate().getPetDataEntry(getLevel() + 1);
		if (pde == null)
			return 0;
